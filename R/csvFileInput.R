...
csvFile <- function(input, output, session, nfactor.limit = 20) {
  ...
  data <- eventReactive(input$file, {
    validate(need((grepl("csv", userFile()$name) == T) | (grepl("xlsx", userFile()$name) == T) | (grepl("sav", userFile()$name) == T) | (grepl("sas7bdat", userFile()$name) == T) |(grepl("dta", userFile()$name) == T) , message = "Please upload csv/xlsx/sav/sas7bdat file"))
  ...
  output$factor <- renderUI({
    selectInput(session$ns("factor_vname"), label = "Các biến có thể thay đổi định dạng",
  ...
  observeEvent(c(data()$factor_original, input$factor_vname), {
    output$binary_check <- renderUI({
      checkboxInput(session$ns("check_binary"), "Tạo biến số nhị phân")
    })

    output$ref_check <- renderUI({
      checkboxInput(session$ns("check_ref"), "Thay đổi giá trị tham chiếu của biến định tính")
    })

    output$subset_check <- renderUI({
      checkboxInput(session$ns("check_subset"), "Giới hạn số liệu")
    })
  })
  ...
  observeEvent(input$check_binary, {
    ...
    output$binary_var <- renderUI({
      ...
      selectInput(session$ns("var_binary"), "Biến số chuyển thành nhị phân",
  ...
  observeEvent(input$check_ref, {
    ...
    output$ref_var <- renderUI({
      ...
      selectInput(session$ns("var_ref"), "Biến số thay đổi giá trị tham chiếu",
    ...
    output$ref_val <- renderUI({
      ...
      for (v in seq_along(input$var_ref)){
        outUI[[v]] <- selectInput(session$ns(paste0("con_ref", v)), paste0("Giá trị tham chiếu: ", input$var_ref[[v]]),
  ...
  observeEvent(input$check_subset, {
    output$subset_var <- renderUI({
      ...
      tagList(
        selectInput(session$ns("var_subset"), "Biến số giới hạn",
    ...
    output$subset_val <- renderUI({
      ...
      for (v in seq_along(input$var_subset)){
        if (input$var_subset[[v]] %in% var.factor){
          ...
          outUI[[v]] <- selectInput(session$ns(paste0("val_subset", v)), paste0("Giá trị giới hạn: ", input$var_subset[[v]]),
                                    choices = varlevel, multiple = T,
                                    selected = varlevel[1])
        } else{
          val <- stats::quantile(data()$data[[input$var_subset[[v]]]], na.rm = T)
          outUI[[v]] <- sliderInput(session$ns(paste0("val_subset", v)), paste0("Khoảng giới hạn: ", input$var_subset[[v]]),
                                    min = val[1], max = val[5],
                                    value = c(val[2], val[4]))
        }
